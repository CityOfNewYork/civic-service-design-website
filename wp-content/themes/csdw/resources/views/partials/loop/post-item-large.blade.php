...
<a href="{{ get_post_permalink( $item->ID ) }}" class="post-item post-item--large hover-state">
  @if( has_post_thumbnail( $item->ID ) )
    <img class="post-item__img post-item__img--large img img-width resize-img-991"
         src="{{ get_the_post_thumbnail_url( $item->ID, 'square_medium' ) }}"
         alt="{{ $item->post_title }}"
         title="{{ $item->post_title }}"
         data-desktop="{{ get_the_post_thumbnail_url( $item->ID, 'square_medium') }}"
         data-mobile="{{ get_the_post_thumbnail_url( $item->ID, 'medium_large' ) }}"
         loading="lazy"
    >
  @endif
